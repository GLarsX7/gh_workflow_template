...
### Added
- **Flutter CI Workflow**: Added `.github/workflows/flutter-ci.yml` for Flutter/Dart projects.
    - **Detection**: Automatically detects `pubspec.yaml`.
    - **Quality**: `dart format` and `flutter analyze`.
    - **Testing**: `flutter test` with coverage.
    - **Build**: Verifies Android build (`flutter build apk --debug`).
- **Documentation**: Enhanced `README.md` with Flutter support details.
- **Versioning**: Added `version.txt` file.
- **Ignore Rules**: Added `.gitignore` for development synchronization.
